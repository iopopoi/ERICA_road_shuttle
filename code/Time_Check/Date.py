...
def deltaminute(delta,start_day = -1): #특정 날짜에서 몇 분 후, 뒤의 날짜를 리턴
    if start_day == -1:
        start_day = today()
    return start_day + timedelta(minutes = delta)


def deltahour(delta,start_day = -1): #특정 날짜에서 몇 시간 후, 뒤의 날짜를 리턴 
    if start_day == -1:
        start_day = today()
    return start_day + timedelta(hours = delta)
